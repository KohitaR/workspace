...
ConlluReader::ConlluReader(){
    Conllu* token;
    token = new Conllu;
    token->setToken(0, FormId_.setObs("ROOT"),
            LemmaId_.setObs("ROOT"), UposTagId_.setObs("ROOT"),
            XposTagId_.setObs("ROOT"), FeatsId_.setObs("ROOT"),
            DeprelId_.setObs("ROOT"), 0, DepsId_.setObs("ROOT"),
            MiscId_.setObs("ROOT"));
    delete token;
}
    ...
    while (getline(ifs, str)) {
        if (regex_match(str, pattern1)) {
            ...
            token->setToken(stoi(string(v[0])), FormId_.setObs(v[1]),
                    ...
                    stoi(string(v[6])) ,DeprelId_.setObs(v[7]), DepsId_.setObs(v[8]),
                    MiscId_.setObs(v[9]));
